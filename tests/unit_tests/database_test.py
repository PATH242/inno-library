...
class TestDatabaseBookFunctions(unittest.TestCase):
    ...
    """, (2, 0))
        self.assertEqual(books, [(1, 'Sample Book', 'Author A', 'Genre A'),
                                 (2, 'Another Book', 'Author B', 'Genre B')])
    ...
    """, ("%Sample%",))
        self.assertEqual(searched_books, [
            (1, 'Sample Book', 'Author A', 'Genre A'),
            (2, 'Another Book', 'Author B', 'Genre B')])
    ...
    """, (genre,))
        self.assertEqual(books_by_genre, [
                         (1, 'Sample Book', 'Author A', 'Genre A'),
                         (2, 'Another Book', 'Author B', 'Genre B')])

    def test_update_book(self):
        db.update_book(1, 'Updated Book', 'Updated Author',
                       'Updated Genre', self.conn)
        self.conn.execute.assert_called_with("""
        UPDATE books
...
class TestDatabaseReadingListFunctions(unittest.TestCase):
    ...
    """, (1,))
        self.assertEqual(
            reading_lists, [(1, 1, 1, 'not_started'),
                            (2, 1, 2, 'in_progress')])
    ...
    """, (1,))
        self.assertEqual(
            readers, [(1, 1, 1, 'not_started'), (2, 1, 2, 'in_progress')])
