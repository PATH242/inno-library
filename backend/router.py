...
def add_to_reading_list(book_id: int, user_id: int = Depends(security.get_user)) -> models.Book:
    reading_list = service.ReadingList(user_id)
    reading_list.add_book(book_id)
    return service.Book.from_db(book_id)

@router.put("/reads", tags=["library"])
def change_reading_status(entry: models.EditReadingList, user_id: int = Depends(security.get_user)) -> models.Book:
    reading_list = service.ReadingList(user_id)
...
def remove_from_reading_list(book_id: int, user_id: int = Depends(security.get_user)) -> str:
    reading_list = service.ReadingList(user_id)
